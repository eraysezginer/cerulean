...
const readiness: { companyId: string; companyName: string; score: number; window: string; note: string }[] = [];
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full min-w-[640px] text-left text-body">
          ...
          <tbody>
            {readiness.length === 0 ? (
              <tr>
                <td colSpan={4} className="p-3 text-text-2">
                  No exit readiness data yet.
                </td>
              </tr>
            ) : readiness.map((r) => (
